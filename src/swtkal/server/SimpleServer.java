...
//		delete(termin, termin.getBesitzer(), besitzerTermine);
	...
	{
		for(Map<String, Vector<Termin>> person  :teilnehmerTermine.values()) {
			for(Vector<Termin> dates : person.values()) {
				for(Termin termin : dates) {
		...
		throw new TerminException("Not found!");
	}
	
	public void delete(int terminID)
	{
		teilnehmerTermine.forEach((person,dates) -> {
			dates.forEach((date,termine) -> {			
				termine.removeIf(temp -> {
					boolean willDelete = temp.getId() == terminID;
					if(willDelete) 
						System.out.println(
						"Remove at Person: '" + person + "'-> [TerminID:" + temp.getId() +
						", Name:' "+ temp.getKurzText() + "']");
